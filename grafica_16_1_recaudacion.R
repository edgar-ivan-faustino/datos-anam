...
# 1. Datos
# Meses y años para trabajarlos diferente
...
df_labels <- df %>%
  ...
  # posición cada etiqueta
  mutate(nudge_y = case_when(
    etiqueta == "Maximo"  ~ 5000,  
    etiqueta == "Minimo"  ~ 5500,  
    etiqueta == "Primero" ~ -4500,  
    etiqueta == "Ultimo"  ~ -4500,  
    TRUE                  ~     0
  ))
...
p <- ggplot(df, aes(x = Mes, y = Valor, group = 1)) +
  ...
  scale_y_continuous(
    breaks = seq(15000, 90000, by = 15000),
    labels = comma_format(),
    expand = c(0, 0)
  ) +
  
  # 
  scale_x_discrete(labels = df$MesCorto, expand = expansion(add = c(1.3, 0.8))) + 
  ...
  coord_cartesian(ylim = c(10000, 96000), clip = "off") +
  
  labs(
    title = NULL, x = NULL,
    y = expression(paste(bold("Recaudación de IVA"), " (MDP)"))
  ) +
  ...
  theme(
    ...
    axis.text.y  = element_text(size = 15, color = "#666666", face = "bold"),
...
# 4. Exportar
ggsave(
  filename = "grafica_16_1recaudacion.svg",
  plot     = p,
  device   = "svg",
  bg       = "transparent",
  width    = 10,
...
ggsave(
  filename = "grafica_16_1recaudacion.png", 
  plot     = p,
  device   = "png",
  bg       = "white", 
  width    = 10,
